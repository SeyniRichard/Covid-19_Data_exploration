...
--Daily percentage of total vaccinated population

With vaccinated (Continent, Location, Date, Population, New_Vaccinations, cumulativevac)
as
(
...
From [Portofolio projects].dbo.CovidDeaths D
Join [Portofolio projects].dbo.CovidVaccinations V
	On D.location = V.location
	and D.date = V.date
where D.continent is not null 
--order by 2,3
)
Select *, (cumulativevac/Population)*100 as PercentPeopleVaccinated
From vaccinated

---Looking at the percentage of infected people in different countries:
